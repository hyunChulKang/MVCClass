...
public class MemberMain {
	...
	private void deleteMember() {
		...
		int cnt = memService.delectMember(memId);
		if(cnt> 0) {
			System.out.println(memId + "회원삭제성공");
		}else {
			System.out.println(memId + "회원삭제 실패!!");
		}
	}
	...
	private void updateMember() {
		...
		mv.setMem_name(memAddr);
		
		int cnt =memService.updateMember(mv);
		
		if(cnt > 0) {
			System.out.println(memId +"회원정보 수정완료");
		}else {
	...
	String memId =scan.nextLine().trim();	//trim 공백 없세기용
